...
The helper-generated hook target follows the existing runtime adapter: POSIX uses the `.sh` target and Windows selects the corresponding PowerShell target during installation. On Linux, exclusive directory publication requires libc to expose `renameat2` and the kernel and filesystem to support `RENAME_NOREPLACE`; when that primitive is unavailable, preparation fails closed without using an overwrite-capable fallback. The preparation and full local journey in this alpha were exercised on macOS; Windows behavior remains covered by the existing adapter tests rather than a new live-host claim.
