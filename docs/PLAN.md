...
`install` owns first private binding and receipt-owned engine updates. Every ready plan emits an opaque `PLAN_HASH` and reviewed remote revision; apply requires that exact token and rebuilds the same facts before writing. First binding, and explicit reacceptance after a valid engine or private-state change invalidates an existing binding, require confirmation, a read-only plan, then tokenized apply. Every target is `identical`, `absent`, `managed-update`, `foreign`, or `indeterminate`; the final two states make the entire plan not ready with zero writes. A managed update requires a valid receipt, matching identity/type/hook ownership, and exact current installed bytes. Apply revalidates complete immutable binding evidence, snapshots every prospective write, and publishes the receipt last. A retained pending marker is diagnostic only: inspect it, clear it only after accepting the retained pre-image evidence, then replan and rerun install; V0.1 does not restore paths from marker or snapshot content. `sync` materializes private state after ordinary Git synchronization, and macOS remains experimental pending C6-8.
